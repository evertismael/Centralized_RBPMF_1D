classdef RBPMF_bsbnd_based
   ...
   methods
      function obj = RBPMF_bsbnd_based()
          '';
      end      
      ...
      function obj = time_update(obj)
          ...
          % MOMENT MATCHING;
          % TO DO: to reduce the computational burden, focuse on the rows
          % where the Pn_k_k is different from zero and reinit the other
          % ones (mean=0 and var=ctte).
          P_kp1_k_ij = obj.Pn_k_k.*(c_ij).*gs.dx;
          sumPj = sum(P_kp1_k_ij,1);
          P_hat_kp1_k_ij = P_kp1_k_ij./sumPj;
